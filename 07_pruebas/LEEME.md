...
| `visual-23-09/` | **C50: la imagen deja de ser una diapositiva.** Muestrario con tres variantes (archivo + frase, archivo + subtítulos, imágenes generadas) sobre los tres Shorts de la semana, montado por el workflow `visual_prueba.yml` de la carpeta. Ver `visual-23-09.md` | **Respondida el 23/09 por el codirector: la A**, con cuatro arreglos y cuatro notas. **Entra en producción el mismo día** (versión 13 del plan, C50): lo que se hizo con cada punto está al final de `visual-23-09.md`. El workflow de la prueba (`visual_prueba.yml`) se retira: su código ya no existe (`visual.py` es ahora el de producción) |
...
| `registro-diario-15-09/` | Diseño de `sincroniza_registro.yml` (C31): llama a `metricas.py --solo-registro` dos veces cada madrugada para que el registro de publicaciones no lleve hasta seis días desactualizado. Deja la función correspondiente (`sincronizar_registro()`) ya escrita y probada en `04_agentes/metricas.py`. Ver su `LEEME.md`. | **Corregido el 22/09: esta fila llevaba días diciendo «falta que lo suba», pero `.github/workflows/sincroniza_registro.yml` está en `origin/main` desde el 15/09 (commit `b68a31b`).** Subido, sí — pero corrigiendo solo cuenta si escribe: desde la última corrección real (`registro: sincronizado con YouTube 2026-09-20`, 09-20 13:14 UTC) no hay ningún commit nuevo de sincronización, y en ese tiempo `MDS-021` pasó a público (17:00 UTC del 21/09) sin que el registro se actualizara en ninguno de los dos crons diarios (`50 8 * * *` y `10 9 * * *` — el propio fichero dice en su cabecera «tres pasadas», pero `on.schedule` solo trae dos). No puedo ver el historial de Actions desde aquí (sin red, sin `gh`) para saber si corre y no encuentra nada que corregir o si falla. Detalle en `05_calendario/bitacora/2026-09-22-revision.md`. |
